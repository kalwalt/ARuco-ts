...
export class CV {
    ...
    static borderFollowing (src: { [x: string]: any; }, pos: number, nbd: number, point: {
        x: any;
        y: any;
    }, hole: boolean, deltas: any[]){
        let contour: any = [], pos1, pos3, pos4, s, s_end, s_prev;
    ...
    static neighborhoodDeltas (width: number): number[]{
        let deltas: number[] = [], len = CV.neighborhood.length, i = 0;
    ...
    static approxPolyDP (contour: any, epsilon: number ): IPoint[] {
        let slice = {start_index: 0, end_index: 0},
            ...
            pt: IPoint, start_pt, end_pt, dist, max_dist, le_eps,
            dx, dy, i, j, k;
